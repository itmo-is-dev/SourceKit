using SourceKit.Generators.Builder.Annotations;
using SourceKit.Generators.Builder.Generators;
using SourceKit.Tests.Common;
using SourceKit.Tests.Common.TestBases;
using Xunit;

namespace SourceKit.Generators.Builder.Tests;

public class BuilderSourceGeneratorTests : GeneratorTestBase<IncrementalBuilderGenerator>
{
    [Fact]
    public async Task ShouldGenerateCorrectBuilderType()
    {
        SourceFile sourceFile = await SourceFile.LoadAsync("SourceKit.Generators.Builder.Samples/SomeQuery.cs");

        var generatedSource = new SourceFile(
            Name: """
            SomeQuery.SourceKit.Generation.Builder.g.cs
            """,
            Content: """
            // <auto-generated>
            // This code was generated by a SourceKit.Generators.Builder code generator.
            // https://github.com/itmo-is-dev/SourceKit
            // </auto-generated>
            #pragma warning disable CS1591
            using SourceKit.Generators.Builder.Annotations;
            using System;
            using System.Collections.Generic;
            using System.Linq;

            #nullable enable
            namespace SourceKit.Generators.Builder.Samples
            {
                public partial record SomeQuery
                {
                    [global::System.Runtime.CompilerServices.MethodImpl(global::System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
                    public static SomeQuery Build(Func<Builder, Builder> action)
                    {
                        return action(new Builder()).Build();
                    }

                    public sealed partial class Builder
                    {
                        private global::System.Collections.Generic.List<global::System.Guid>? _ids;
                        private int _count;
                        private int? _orderById;
                        public Builder()
                        {
                            _count = default;
                            _orderById = default;
                        }

                        [InitializesPropertyAttribute(nameof(Ids))]
                        public Builder WithId(global::System.Guid element)
                        {
                            _ids ??= [];
                            _ids.Add(element);
                            return this;
                        }

                        [InitializesPropertyAttribute(nameof(Ids))]
                        public Builder WithIds(global::System.Collections.Generic.IEnumerable<global::System.Guid> elements)
                        {
                            _ids ??= [];
                            _ids.AddRange(elements);
                            return this;
                        }

                        [InitializesPropertyAttribute(nameof(Count))]
                        public Builder WithCount(int value)
                        {
                            _count = value;
                            return this;
                        }

                        [InitializesPropertyAttribute(nameof(OrderById))]
                        public Builder WithOrderById(int? value)
                        {
                            _orderById = value;
                            return this;
                        }

                        public SomeQuery Build()
                        {
                            return new SomeQuery(_ids?.Distinct()?.ToArray() ?? [], _count, _orderById);
                        }
                    }
                }
            }
            """);

        await GeneratorTest
            .WithSource(sourceFile)
            .WithGeneratedSource(generatedSource)
            .WithAdditionalReference(typeof(GenerateBuilderAttribute).Assembly)
            .Build()
            .RunAsync();
    }

    [Fact]
    public async Task ShouldGenerateCorrectBuilderType_WhenPropertyIsArray()
    {
        SourceFile sourceFile = await SourceFile.LoadAsync("SourceKit.Generators.Builder.Samples/ArrayQuery.cs");

        var generatedSource = new SourceFile(
            Name: """
            ArrayQuery.SourceKit.Generation.Builder.g.cs
            """,
            Content: """
            // <auto-generated>
            // This code was generated by a SourceKit.Generators.Builder code generator.
            // https://github.com/itmo-is-dev/SourceKit
            // </auto-generated>
            #pragma warning disable CS1591
            using SourceKit.Generators.Builder.Annotations;
            using System;
            using System.Collections.Generic;
            using System.Linq;

            #nullable enable
            namespace SourceKit.Generators.Builder.Samples
            {
                public partial record ArrayQuery
                {
                    [global::System.Runtime.CompilerServices.MethodImpl(global::System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
                    public static ArrayQuery Build(Func<Builder, Builder> action)
                    {
                        return action(new Builder()).Build();
                    }

                    public sealed partial class Builder
                    {
                        private global::System.Collections.Generic.List<global::System.Guid>? _ids;
                        public Builder()
                        {
                        }

                        [InitializesPropertyAttribute(nameof(Ids))]
                        public Builder WithId(global::System.Guid element)
                        {
                            _ids ??= [];
                            _ids.Add(element);
                            return this;
                        }

                        [InitializesPropertyAttribute(nameof(Ids))]
                        public Builder WithIds(global::System.Collections.Generic.IEnumerable<global::System.Guid> elements)
                        {
                            _ids ??= [];
                            _ids.AddRange(elements);
                            return this;
                        }

                        public ArrayQuery Build()
                        {
                            return new ArrayQuery(_ids?.Distinct()?.ToArray() ?? []);
                        }
                    }
                }
            }
            """);

        await GeneratorTest
            .WithSource(sourceFile)
            .WithGeneratedSource(generatedSource)
            .WithAdditionalReference(typeof(GenerateBuilderAttribute).Assembly)
            .Build()
            .RunAsync();
    }

    [Fact]
    public async Task ShouldGenerateCorrectBuilderType_WhenPropertyHasCustomType()
    {
        SourceFile sourceFile = await SourceFile.LoadAsync("SourceKit.Generators.Builder.Samples/ArrayQuery.cs");

        var generatedSource = new SourceFile(
            Name: """
            ArrayQuery.SourceKit.Generation.Builder.g.cs
            """,
            Content: """
            // <auto-generated>
            // This code was generated by a SourceKit.Generators.Builder code generator.
            // https://github.com/itmo-is-dev/SourceKit
            // </auto-generated>
            #pragma warning disable CS1591
            using SourceKit.Generators.Builder.Annotations;
            using System;
            using System.Collections.Generic;
            using System.Linq;

            #nullable enable
            namespace SourceKit.Generators.Builder.Samples
            {
                public partial record ArrayQuery
                {
                    [global::System.Runtime.CompilerServices.MethodImpl(global::System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
                    public static ArrayQuery Build(Func<Builder, Builder> action)
                    {
                        return action(new Builder()).Build();
                    }

                    public sealed partial class Builder
                    {
                        private global::System.Collections.Generic.List<global::System.Guid>? _ids;
                        public Builder()
                        {
                        }

                        [InitializesPropertyAttribute(nameof(Ids))]
                        public Builder WithId(global::System.Guid element)
                        {
                            _ids ??= [];
                            _ids.Add(element);
                            return this;
                        }

                        [InitializesPropertyAttribute(nameof(Ids))]
                        public Builder WithIds(global::System.Collections.Generic.IEnumerable<global::System.Guid> elements)
                        {
                            _ids ??= [];
                            _ids.AddRange(elements);
                            return this;
                        }

                        public ArrayQuery Build()
                        {
                            return new ArrayQuery(_ids?.Distinct()?.ToArray() ?? []);
                        }
                    }
                }
            }
            """);

        await GeneratorTest
            .WithSource(sourceFile)
            .WithGeneratedSource(generatedSource)
            .WithAdditionalReference(typeof(GenerateBuilderAttribute).Assembly)
            .Build()
            .RunAsync();
    }

    [Fact]
    public async Task ShouldGenerateCorrectBuilderType_WhenPropertyHasBuilderConstructorParameterAttribute()
    {
        SourceFile sourceFile = await SourceFile.LoadAsync(
            "SourceKit.Generators.Builder.Samples/BuilderConstructorParameter/IntegerConstructorParameterQuery.cs");

        var generatedSource = new SourceFile(
            Name: """
            IntegerConstructorParameterQuery.SourceKit.Generation.Builder.g.cs
            """,
            Content: """
            // <auto-generated>
            // This code was generated by a SourceKit.Generators.Builder code generator.
            // https://github.com/itmo-is-dev/SourceKit
            // </auto-generated>
            #pragma warning disable CS1591
            using SourceKit.Generators.Builder.Annotations;
            using System;
            using System.Collections.Generic;
            using System.Linq;
            
            #nullable enable
            namespace SourceKit.Generators.Builder.Samples.BuilderConstructorParameter
            {
                public partial record IntegerConstructorParameterQuery
                {
                    [global::System.Runtime.CompilerServices.MethodImpl(global::System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
                    public static IntegerConstructorParameterQuery Build(int? Id, IEnumerable<int> Values, Func<Builder, Builder> action)
                    {
                        return action(new Builder(Id, Values)).Build();
                    }
            
                    public sealed partial class Builder
                    {
                        private int? _id;
                        private global::System.Collections.Generic.List<int> _values;
                        private global::System.Collections.Generic.List<int>? _notParameterValues;
                        private string? _value;
                        public Builder(int? Id, IEnumerable<int> Values)
                        {
                            _id = Id;
                            _values = [..Values];
                            _value = string.Empty;
                        }
            
                        [InitializesPropertyAttribute(nameof(Values))]
                        public Builder WithValue(int element)
                        {
                            _values.Add(element);
                            return this;
                        }
            
                        [InitializesPropertyAttribute(nameof(Values))]
                        public Builder WithValues(global::System.Collections.Generic.IEnumerable<int> elements)
                        {
                            _values.AddRange(elements);
                            return this;
                        }
            
                        [InitializesPropertyAttribute(nameof(NotParameterValues))]
                        public Builder WithNotParameterValue(int element)
                        {
                            _notParameterValues ??= [];
                            _notParameterValues.Add(element);
                            return this;
                        }
            
                        [InitializesPropertyAttribute(nameof(NotParameterValues))]
                        public Builder WithNotParameterValues(global::System.Collections.Generic.IEnumerable<int> elements)
                        {
                            _notParameterValues ??= [];
                            _notParameterValues.AddRange(elements);
                            return this;
                        }
            
                        [InitializesPropertyAttribute(nameof(Id))]
                        public Builder WithId(int? value)
                        {
                            _id = value;
                            return this;
                        }
            
                        [InitializesPropertyAttribute(nameof(Value))]
                        public Builder WithValue(string value)
                        {
                            _value = value;
                            return this;
                        }
            
                        public IntegerConstructorParameterQuery Build()
                        {
                            return new IntegerConstructorParameterQuery(_id, _values.Distinct().ToArray() ?? [], _notParameterValues?.Distinct()?.ToList() ?? [], _value ?? throw new System.ArgumentNullException(nameof(_value)));
                        }
                    }
                }
            }
            """);

        await GeneratorTest
            .WithSource(sourceFile)
            .WithGeneratedSource(generatedSource)
            .WithAdditionalReference(typeof(GenerateBuilderAttribute).Assembly)
            .Build()
            .RunAsync();
    }
}
